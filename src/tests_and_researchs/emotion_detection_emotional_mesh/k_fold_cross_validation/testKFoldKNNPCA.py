...
def test_values(k_values, pca_components, X_train, X_test, y_train, y_test):
    accuracy_matrix_iteration = np.zeros((len(pca_components), len(k_values)))
...
for train_index, test_index in skf.split(X, y):
    ...
    accuracy_matrix_iteration = test_values(k_values, pca_components,
                                                X_train, X_test,
                                                y_train, y_test)
    accuracy_matrix[split_index] = accuracy_matrix_iteration
    split_index += 1

accuracy_matrix_means = mean_of_iterations(accuracy_matrix, len(k_values), len(pca_components))
